...
from fastapi_2fa_example.auth.schemas import (
    LoginRequest,
    RegisterRequest,
    TokenType,
    TwoFARequest,
...
class TestVerify2FA:
    ...
    async def test_verify_2fa_wrong_token_type(
        self, client: AsyncClient, redis: Redis, random_2fa_user: User
    ) -> None:
        login_request = LoginRequest(
            email=random_2fa_user.email,
    ...
    async def test_verify_2fa_no_otp(
        self, client: AsyncClient, redis: Redis, random_2fa_user: User
    ) -> None:
        tmp_token = create_jwt_token(
            user_id=random_2fa_user.id,
        ...
        two_fa_request = TwoFARequest(tmp_token=tmp_token, otp="000000")
